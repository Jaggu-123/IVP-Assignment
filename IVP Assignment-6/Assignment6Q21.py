...
def erosion(image, num_of_rows, num_of_cols):
    ...
    for x in range(num_of_rows-2):
        for y in range(num_of_cols):
            ...
            minimum = np.amin(subtracted_array)

            if minimum <= 0:
                eroded_image[x+1][y] = 0
            else:
                eroded_image[x+1][y] = minimum
    ...
    for x in range(num_of_rows-2):
        for y in range(num_of_cols):
            ...
            if minimum <= 0:
                eroded_image2[x+1][y] = 0
            else:
                eroded_image2[x+1][y] = minimum
    ...
    for x in range(num_of_rows-2):
        for y in range(num_of_cols):
            ...
            if minimum <= 0:
                eroded_image3[x+1][y] = 0
            else:
                eroded_image3[x+1][y] = minimum

    cv2.imshow("Original Image", image)
    cv2.imshow("Eroded Image", eroded_image)
    cv2.imshow("Eroded Image 2", eroded_image2)
    cv2.imshow("Eroded Image 3", eroded_image3)
    cv2.waitKey(0)
